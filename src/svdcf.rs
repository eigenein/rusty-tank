...
fn main() {
    ...
    let (_, train_error) = helpers::evaluate(&model, &train_matrix, &train_matrix, helpers::identity);
    println!("Train error: {0:.6}.", train_error);
    let (_, test_error) = helpers::evaluate(&model, &train_matrix, &test_matrix, helpers::identity);
